...
private fun getMccMnc(identity: CellIdentityLte): Pair<String, String> {
    return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
        ...
        Pair(identity.mcc.toString(), identity.mnc.toString())
    }
}
...
suspend fun getCellularInfo(context: Context): String = withContext(Dispatchers.IO) {
        ...
        primaryCell?.let { cell ->
            when (cell) {
                is CellInfoLte -> {
                    ...
                    builder.appendLine("  Signal Strength: ${cell.cellSignalStrength.level} dBm")
                }
                is CellInfoGsm -> {
                    val identity = cell.cellIdentity
                    val (mcc, mnc) = getMccMnc(identity)
